...
static int jtagspi_cmd(struct flash_bank *bank, uint8_t cmd, uint32_t *addr,
...
static int jtagspi_probe(struct flash_bank *bank) {
  ...
  LOG_INFO("Found flash device \'%s\' (ID 0x%08" PRIx32 ")", info->dev->name,
           info->dev->device_id);
...
static int jtagspi_sector_erase(struct flash_bank *bank, int sector) {
  ...
  if (retval != ERROR_OK)
    return retval;
  jtagspi_cmd(bank, info->dev->erase_cmd, &bank->sectors[sector].offset, NULL,
              0);
...
static int jtagspi_protect(struct flash_bank *bank, int set, int first,
                           int last) {
  int sector;
...
static int jtagspi_protect_check(struct flash_bank *bank) { return ERROR_OK; }

static int jtagspi_read(struct flash_bank *bank, uint8_t *buffer,
...
static int jtagspi_page_write(struct flash_bank *bank, const uint8_t *buffer,
                              uint32_t offset, uint32_t count) {
  int retval;
  ...
  if (retval != ERROR_OK)
    return retval;
  jtagspi_cmd(bank, SPIFLASH_PAGE_PROGRAM, &offset, (uint8_t *)buffer,
              count * 8);
  return jtagspi_wait(bank, JTAGSPI_MAX_TIMEOUT);
}

static int jtagspi_write(struct flash_bank *bank, const uint8_t *buffer,
                         uint32_t offset, uint32_t count) {
  struct jtagspi_flash_bank *info = bank->driver_priv;
  int retval;
  uint32_t n;
