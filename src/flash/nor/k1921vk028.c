...
COMMAND_HANDLER(k1921vk028_handle_read_command)
{
    ...
    if (type == MFLASH)
        ...
        command_print(CMD_CTX,  "Read BFLASH %s region:\n"
                                "    addr = 0x%04x, data = 0x%04x\n"
                                "    addr = 0x%04x, data = 0x%04x\n"
                                "    addr = 0x%04x, data = 0x%04x\n"
                                "    addr = 0x%04x, data = 0x%04x", region ? "NVR" : "main",
...
COMMAND_HANDLER(k1921vk028_handle_write_command)
{
    ...
    if (type == MFLASH)
        ...
        command_print(CMD_CTX,  "Write BFLASH %s region%s:\n"
                                "    addr = 0x%04x, data = 0x%04x\n"
                                "    addr = 0x%04x, data = 0x%04x\n"
                                "    addr = 0x%04x, data = 0x%04x\n"
                                "    addr = 0x%04x, data = 0x%04x", region ? "NVR" : "main",
...
COMMAND_HANDLER(k1921vk028_handle_mass_erase_command)
{
    ...
    command_print(CMD_CTX, "Mass erase %s %s region\n"
...
COMMAND_HANDLER(k1921vk028_handle_erase_command)
{
    ...
    command_print(CMD_CTX, "Erase %s %s region sectors %d through %d\n"
                           "Please wait ... ", type ? "BFLASH" : "MFLASH",
...
COMMAND_HANDLER(k1921vk028_handle_protect_command)
{
    ...
    command_print(CMD_CTX, "Try to %s %s %s region write protection\n"
...
COMMAND_HANDLER(k1921vk028_handle_srv_erase_command)
{
    ...
    command_print(CMD_CTX, "done! Power on reset cycle and SRV low are required for the return to normal operation mode.");
...
COMMAND_HANDLER(k1921vk028_handle_driver_info_command)
{
    ...
    command_print(CMD_CTX, "k1921vk028 flash driver\n"
                           "version: %d.%d\n"
...
static int k1921vk028_erase(struct flash_bank *bank, int first, int last)
{
    struct k1921vk028_flash_bank *k1921vk028_info = bank->driver_priv;
    struct target *target = bank->target;
...
static int k1921vk028_protect(struct flash_bank *bank, int set, int first, int last)
{
    (void)first;
    (void)last;
    struct target *target = bank->target;
...
static int get_k1921vk028_info(struct flash_bank *bank, char *buf, int buf_size)
{
    struct k1921vk028_flash_bank *k1921vk028_info = bank->driver_priv;
